...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    RKObjectManager *objectManager = [RKObjectManager managerWithBaseURLString:@"http://blcktrckr.herokuapp.com/services/rest"];
    if (objectManager) {
        NSLog(@"AppDelegate:RestKit initialized with base URL: %@", @"http://blcktrckr.herokuapp.com/services/rest");
    }
    
    [[RKObjectManager sharedManager].router routeClass:[Trip class]
                                        toResourcePath:@"/trip"
                                             forMethod:RKRequestMethodPOST];
    
    [[RKObjectManager sharedManager].router routeClass:[BTPoint class]
                                        toResourcePath:@"/point"
                                             forMethod:RKRequestMethodPOST];
    
    //Create parser to parse stuff returned from json post, set it to read type "text/plain"
                      ...
                      error:(NSError *)error
{
    [FBRequestConnection startForMeWithCompletionHandler:^(FBRequestConnection *connection,
                                       id<FBGraphUser> user,
                                                           NSError *error) {
        self.dataHandler.facebookId = [NSNumber numberWithInt:[(user.id) intValue]];
    } ];
...
#warning save user history here!!!!
